...
{
	public class SoaringWisp : ModNPC
	{
		...
		public override void AI()
		{
			...
			if (npc.ai[0] < 180)
			{
				...
				int index = Dust.NewDust(npc.position, npc.width, npc.height, 176, 0.0f, 0.0f, 0, new Color(), 0.75f);
		...
		{
			Item.NewItem((int)npc.position.X, (int)npc.position.Y, npc.width, npc.height, mod.ItemType("SoaringEnergy"), Main.rand.Next(2, 4));
		}
	}
}
