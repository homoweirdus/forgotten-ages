...
{
    public class ForestPortalFriendly : ModProjectile
    {
		int counter = 0;
		int counter2 = 0;
		int dist = 1000;
        ...
        public override void AI()
        {
			...
			if (counter <= 50)
			{
				if (Main.rand.Next(4) == 0)
				{
					...
					Dust dust = Main.dust[Dust.NewDust((projectile.Center - (spinningpoint * 30f)), 0, 0, 163, 0.0f, 0.0f, 0, new Color(), 1f)];
					dust.noGravity = true;
			...
			if ((double) counter <= 90.0)
			{
				...
				{
					int num5 = Dust.NewDust(projectile.position, projectile.width, projectile.height, 163, 0f, 0f, 240, default(Color), 0.5f);
			...
			if (projectile.scale == 1f)
			{
				...
				if (counter2 >= 30)
				{
					...
					if (AnyNPCs() == true)
					{
						Vector2 v = (Main.npc[closest].Center - projectile.Center);
						v.Normalize();
						v *= 16;
						Projectile.NewProjectile(projectile.Center.X, projectile.Center.Y, v.X, v.Y, mod.ProjectileType("Leaf"), projectile.damage, 5f, projectile.owner);
						counter2 = 0;
					}
				}
			}
