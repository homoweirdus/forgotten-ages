...
{
	public class ThornballFlail : ModProjectile
	{
		int index1 = -1;
		float rotate;
		...
		{
			int dust;
			dust = Dust.NewDust(projectile.Center + projectile.velocity, 0, 0, 32, 0f, 0f);
		...
		public override bool PreDraw(SpriteBatch spriteBatch, Color lightColor)
		{
			...
			while (flag)
			{
				...
				else
				{
					...
					Main.spriteBatch.Draw(mod.GetTexture("Projectiles/Chain/DuneChain"), new Vector2(vector2.X - Main.screenPosition.X, vector2.Y - Main.screenPosition.Y), new Microsoft.Xna.Framework.Rectangle?(new Microsoft.Xna.Framework.Rectangle(0, 0, 6, 14)), color, rotation, new Vector2((float) 10 * 0.5f, (float) 16 * 0.5f), 1f, SpriteEffects.None, 0.0f);
				}
			}
			return true;
		}
