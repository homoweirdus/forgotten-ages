	...
	public class SpinalBolt : ModProjectile
	{
		...
		public override void AI()
		{
			for (int index1 = 0; index1 < 2; ++index1)
			{
				...
				int num3 = 4;
				int index2 = Dust.NewDust(new Vector2(projectile.position.X + (float) num3, projectile.position.Y + (float) num3), projectile.width - num3 * 2, projectile.height - num3 * 2, 5, 0.0f, 0.0f, 0, default(Color), 1f);
		...
		public override void OnHitNPC(NPC target, int damage, float knockback, bool crit)
		{
			...
			int kek = Projectile.NewProjectile(projectile.position.X, projectile.position.Y, 0f, -4f, mod.ProjectileType("SpinalFountain"), projectile.damage, 1f, projectile.owner);
		}
	}
}