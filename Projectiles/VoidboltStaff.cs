...
{
	public class VoidboltStaff : ModProjectile
	{
		int kek;
			...
			if (flag6 && Main.myPlayer == projectile.owner)
			{
				...
				if (flag8)
				{
					if (projectile.ai[0] == 360f)
					{
						...
						int num22 = Projectile.NewProjectile(center.X, center.Y, vector15.X, vector15.Y, mod.ProjectileType("ShadowbeamBig"), num21, projectile.knockBack, projectile.owner, 0f, (float)projectile.whoAmI);
						kek = num22;
						projectile.netUpdate = true;
					}
					else if (flag5)
					{
						...
						if (!projectile1.active || projectile1.type != mod.ProjectileType("ShadowbeamBig"))
						{
							projectile.Kill();
							return;
						}
				...
				else
				{
					if (!flag5)
					{
						...
						int num25 = (num24 < 1f) ? projectile.damage : ((int)((float)projectile.damage * 8f));
						Projectile.NewProjectile(center2.X, center2.Y, vector16.X, vector16.Y, num23, num25, projectile.knockBack, projectile.owner, 0, 0);
					}
					projectile.Kill();
				}
			}
		...
		{
			if (projectile.ai[0] > 360f && Main.projectile[kek].type == mod.ProjectileType("ShadowbeamBig"))
			{
