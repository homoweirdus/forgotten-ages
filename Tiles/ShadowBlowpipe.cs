...
{
    public class ShadowBlowpipe : GlobalTile
    {
        public override bool Drop(int i, int j, int type)
        {
            ...
            if (type == TileID.Plants || type == TileID.Plants2)
            {
                ...
                if (WorldGen.genRand.Next(2) == 0 && Main.player[(int)Player.FindClosest(new Vector2((float)(i * 16), (float)(j * 16)), 16, 16)].HasItem(mod.ItemType("ShadowBlowpipe")))
                {
                    // Spawn a seed!
